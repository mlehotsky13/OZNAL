...
treeTfPredictions[["earn"]] <- predict(treeTfEarn, newdata = dataframeDtmTest)
treeTfPredictions[["acq"]] <- predict(treeTfAcq, newdata = dataframeDtmTest)
treeTfPredictions[["moneyfx"]] <- predict(treeTfMoneyfx, newdata = dataframeDtmTest)
treeTfPredictions[["crude"]] <- predict(treeTfCrude, newdata = dataframeDtmTest)
treeTfPredictions[["grain"]] <- predict(treeTfGrain, newdata = dataframeDtmTest)
treeTfPredictions[["trade"]] <- predict(treeTfTrade, newdata = dataframeDtmTest)
treeTfPredictions[["interest"]] <- predict(treeTfInterest, newdata = dataframeDtmTest)
treeTfPredictions[["wheat"]] <- predict(treeTfWheat, newdata = dataframeDtmTest)
treeTfPredictions[["ship"]] <- predict(treeTfShip, newdata = dataframeDtmTest)
treeTfPredictions[["corn"]] <- predict(treeTfCorn, newdata = dataframeDtmTest)

treeTfResults2 <- createResultsDF(treeTfPredictions);

treeTfidfPredictions = list()
treeTfidfPredictions[["earn"]] <- predict(treeTfidfEarn, newdata = dataframeWeightedDtmTest)
treeTfidfPredictions[["acq"]] <- predict(treeTfidfAcq, newdata = dataframeWeightedDtmTest)
treeTfidfPredictions[["moneyfx"]] <- predict(treeTfidfMoneyfx, newdata = dataframeWeightedDtmTest)
treeTfidfPredictions[["crude"]] <- predict(treeTfidfCrude, newdata = dataframeWeightedDtmTest)
treeTfidfPredictions[["grain"]] <- predict(treeTfidfGrain, newdata = dataframeWeightedDtmTest)
treeTfidfPredictions[["trade"]] <- predict(treeTfidfTrade, newdata = dataframeWeightedDtmTest)
treeTfidfPredictions[["interest"]] <- predict(treeTfidfInterest, newdata = dataframeWeightedDtmTest)
treeTfidfPredictions[["wheat"]] <- predict(treeTfidfWheat, newdata = dataframeWeightedDtmTest)
treeTfidfPredictions[["ship"]] <- predict(treeTfidfShip, newdata = dataframeWeightedDtmTest)
treeTfidfPredictions[["corn"]] <- predict(treeTfidfCorn, newdata = dataframeWeightedDtmTest)
...
# for 10 specific topic columns
neuralNetTfEarn  <- train(earn ~ . , data=dataframeDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfAcq  <- train(acq ~ . , data=dataframeDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfMoneyfx  <- train(moneyfx ~ . , data=dataframeDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfCrude  <- train(crude ~ . , data=dataframeDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfGrain  <- train(grain ~ . , data=dataframeDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfTrade  <- train(trade ~ . , data=dataframeDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfInterest  <- train(interest ~ . , data=dataframeDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfWheat  <- train(wheat ~ . , data=dataframeDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfShip  <- train(ship ~ . , data=dataframeDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfCorn  <- train(corn ~ . , data=dataframeDtmTrain, trControl = ctrl, method = "dnn")

neuralNetTfidfEarn  <- train(earn ~ . , data=dataframeWeightedDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfidfAcq  <- train(acq ~ . , data=dataframeWeightedDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfidfMoneyfx  <- train(moneyfx ~ . , data=dataframeWeightedDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfidfCrude  <- train(crude ~ . , data=dataframeWeightedDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfidfGrain  <- train(grain ~ . , data=dataframeWeightedDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfidfTrade  <- train(trade ~ . , data=dataframeWeightedDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfidfInterest  <- train(interest ~ . , data=dataframeWeightedDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfidfWheat  <- train(wheat ~ . , data=dataframeWeightedDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfidfShip  <- train(ship ~ . , data=dataframeWeightedDtmTrain, trControl = ctrl, method = "dnn")
neuralNetTfidfCorn  <- train(corn ~ . , data=dataframeWeightedDtmTrain, trControl = ctrl, method = "dnn")

neuralNetTfPredictions = list()
neuralNetTfPredictions[["earn"]] <- predict(neuralNetTfEarn, newdata = dataframeDtmTest)
neuralNetTfPredictions[["acq"]] <- predict(neuralNetTfAcq, newdata = dataframeDtmTest)
neuralNetTfPredictions[["moneyfx"]] <- predict(neuralNetTfMoneyfx, newdata = dataframeDtmTest)
neuralNetTfPredictions[["crude"]] <- predict(neuralNetTfCrude, newdata = dataframeDtmTest)
neuralNetTfPredictions[["grain"]] <- predict(neuralNetTfGrain, newdata = dataframeDtmTest)
neuralNetTfPredictions[["trade"]] <- predict(neuralNetTfTrade, newdata = dataframeDtmTest)
neuralNetTfPredictions[["interest"]] <- predict(neuralNetTfInterest, newdata = dataframeDtmTest)
neuralNetTfPredictions[["wheat"]] <- predict(neuralNetTfWheat, newdata = dataframeDtmTest)
neuralNetTfPredictions[["ship"]] <- predict(neuralNetTfShip, newdata = dataframeDtmTest)
neuralNetTfPredictions[["corn"]] <- predict(neuralNetTfCorn, newdata = dataframeDtmTest)

neuralNetTfResults2 <- createResultsDF(neuralNetTfPredictions)

neuralNetTfidfPredictions = list()
neuralNetTfidfPredictions[["earn"]] <- predict(neuralNetTfidfEarn, newdata = dataframeWeightedDtmTest)
neuralNetTfidfPredictions[["acq"]] <- predict(neuralNetTfidfAcq, newdata = dataframeWeightedDtmTest)
neuralNetTfidfPredictions[["moneyfx"]] <- predict(neuralNetTfidfMoneyfx, newdata = dataframeWeightedDtmTest)
neuralNetTfidfPredictions[["crude"]] <- predict(neuralNetTfidfCrude, newdata = dataframeWeightedDtmTest)
neuralNetTfidfPredictions[["grain"]] <- predict(neuralNetTfidfGrain, newdata = dataframeWeightedDtmTest)
neuralNetTfidfPredictions[["trade"]] <- predict(neuralNetTfidfTrade, newdata = dataframeWeightedDtmTest)
neuralNetTfidfPredictions[["interest"]] <- predict(neuralNetTfidfInterest, newdata = dataframeWeightedDtmTest)
neuralNetTfidfPredictions[["wheat"]] <- predict(neuralNetTfidfWheat, newdata = dataframeWeightedDtmTest)
neuralNetTfidfPredictions[["ship"]] <- predict(neuralNetTfidfShip, newdata = dataframeWeightedDtmTest)
neuralNetTfidfPredictions[["corn"]] <- predict(neuralNetTfidfCorn, newdata = dataframeWeightedDtmTest)

neuralNetTfidfResults2 <- createResultsDF(neuralNetTfidfPredictions)